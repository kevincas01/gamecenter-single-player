...
import {  checkCorrect, checkWrongPlacement, countLetterOccurrences, generateWordSet } from '../lib/wordleUtils';
...
const KeyboardButton = ({ value }) => {
    ...
    return (
        <div className={`keyboard-letter ${state}`} onClick={()=>handleLetterSelect(value)}>
        {value}
      </div>
    );
  };
  ...
  const KeyboardGrid = () => {
    ...
    const { handleLetterSelect,handleLetterDelete,handleWordEnter} = React.useContext(BoardContext);
      ...
      <div className='keyboard-grid' >
        <div className='keyboard-letters' onKeyDown={handleKeyboardPress}>
          {keyboardLetters1.map((keyLetter, index) => (
            <KeyboardButton key={index} value={keyLetter} handleClick={handleLetterSelect} />
          ))}
        </div>
        ...
        <div className='keyboard-letters'>
          <div className='keyboard-letter special' onClick={handleWordEnter}>Enter</div>
          {keyboardLetters3.map((keyLetter, index) => (
            <KeyboardButton key={index} value={keyLetter} handleClick={handleLetterSelect}/>
          ))}
          <div className='keyboard-letter special'onClick={handleLetterDelete} >Delete</div>
        </div>
      </div>
    );
  };
   ...
   const handleLetterSelect=(keyVal)=>{
        ...
        for (let i = 0; i < word.length; i++) {
            ...
            if (word[i] in newSet) {
                ...
                if(newSet[word[i]] === "wrong-place"){
                    state = word[i].toUpperCase() === correctWord[i].toUpperCase()?"correct":"wrong-place";
                    newSet[word[i]] = state;
                }
                continue
            }
            ...
            wrongPlace = checkWrongPlacement(correctWordLetterCount, word, word[i], i);
    ...
    return (
        <div className='wordle-container'>
            ...
            :(
            <BoardContext.Provider value={{boardState,setBoardState,correctWord, correctWordLetterCount,attemptNumber,setAttemptNumber,positionNumber,setPositionNumber,handleLetterSelect,handleLetterDelete,handleWordEnter,letterSetUsed}}>
